...
G = np.hstack((A.ravel(),B.ravel(),C.ravel(),D.ravel(),E.ravel(),F.ravel()))
m = -1
for j in [A, B, C, D, E, F]:
    m += 1
    for num in G[49*m:(49*m+48)]:
            for i in range(2, num):
                if num % i == 0:
                    break
            else:
                if len(np.argwhere(j==num)) > 0 and m < 6:
                    print('%s'%['A','B','C','D','E','F'][m], num, np.argwhere(j==num))